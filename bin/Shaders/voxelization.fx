...
//
// FX file for voxelizating 3D models to voxels
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
// Geometry shader to adjust triangle transform to make sure
// triangles are ready for voxelization
//--------------------------------------------------------------------------------------
[maxvertexcount(3)]
void GS_MAIN(triangle VS_OUT input[3], inout TriangleStream<GS_OUT> triStream)
{
	...
	// Rasterizing triangles may clip some parts of triangle or even an entire triangle
	// To make sure that every triangle is complete, the shader adjusts triangle positions
	// Rotate triangle to be opposite with view direction to avoid clipping problem

	// Using float3(0,0,1) as view direction
	// Find the orthogonal direction of triangle
	...
	if (dot(v, v) > 0) {
		...
		// Creating a rotation matrix to rotate the triangle to the opposite view direction
	...
	// Build a bounding box of triangle in order to control the density of pixels that a triangle can produce
	...
	for (int i = 0; i < 3; i++)
	{
		...
		output.posV = input[i].posV;	//send view-space voxel positions
...
//--------------------------------------------------------------------------------------
// Pixel shader for saving data to shader resource(Texture3D)
//--------------------------------------------------------------------------------------
float4 PS_MAIN(GS_OUT pIn) : SV_Target
{
	// Using view-space position to access an address of Texture3D
	...
	// A trivial return-value because I don't set render target
